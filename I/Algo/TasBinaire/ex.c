...
void InsertValue(BinaryHeap *heap, int value)
{
    ...
    while(heap->array[current_index] > heap->array[parent_index]) {
...
int ExtractMax(BinaryHeap *heap, int *res)
{
    ...
    int current_index = 0, right_child_index, left_child_index, max_index;
    ...
    *res = root;
    ...
    while(1) {
        if(has_left_child(current_index, heap->filled)) {
            if(has_right_child(current_index, heap->filled)) {
                right_child_index = right_child(current_index);
                left_child_index = left_child(current_index);
                max_index = max(heap->array, right_child_index, left_child_index);
                ...
                left_child_index = left_child(current_index);
                max_index = max(heap->array, current_index, left_child_index);
                if(max_index == left_child_index) {
                    swap(&heap->array[left_child_index], &heap->array[current_index]);
                } 
